...
2. 左側のメニューバーから「クラスター」を選び、「クラスターの作成」をクリックする。
3. クラスター名を入力する。
...
**※**　本分析に使用したPC環境が最新ではなかったため、「インフラストラクチャの要件」セクションの「オペレーションシステム/アーキテクチャ」が選べる箇所を探し、アーキテクチャのプルダウンを`ARM64`に変更した。
...
* **アプリの確認方法**:ステータスが `実行中` になったら、その他タスクIDを開き、「設定」または「ネットワーク」項目の「**パブリック IP**」の数字から始まるURLをコピーし、ブラウザのURL欄にペーストしてアクセスする。なお、その際、以下のように少し訂正する必要がある。
```
http://xx.xx.xxx.xxx:8501
```

## AWSリソースの削除
AWSは有料コンテンツのため、ここで片付けをしておかないと後悔するだろう。

### タスクの停止
1. 検索窓で`ECS`と検索し、「クラスター」から作成したクラスターを開く。
2. 「タスク」タブを開き、ステータスが`実行中`になっているタスクの左側のチェックボックスにチェックを入れる。
3. 右上の「停止」をクリックし、確認画面でもう一度「停止」を押す。

### クラスターの削除
1. クラスターの詳細画面の右上にある「クラスターの削除」をクリックする。
2. 指示の内容に従い、「削除」をクリックする。

### タスク定義の無効化
1. ECS画面の左側のメニューバーから「タスク定義」を開く。
2. 作成したタスクをクリックして開く。
3. 表示されているリビジョンにチェックをいれる。
4. 右上の「アクション」から「登録解除」をクリックする。

### ECRの倉庫とDockerイメージの削除
1. 検索窓で`ECR`と検索し、リポジトリ一覧から倉庫名の左側の丸いラジオボタンを選択する。
2. 右上の「削除」をクリックし、指示に従い完全に削除する。

### セキュリティグループの削除
1. 検索窓で`EC2`と検索し、開く。
2. 左側のメニューバーの下の方にある「ネットワーク＆セキュリティ」の中の「セキュリティグループ」をクリックする。
3. 全選択し、右上の「アクション」から「セキュリティグループの削除」をクリックして削除する。